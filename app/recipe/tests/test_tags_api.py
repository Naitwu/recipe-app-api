...
def create_user(email='user@example.comn', password='testpass123'):
...
class PublicTagsApiTests(TestCase):
    ...
    def test_login_required(self):
        """Test unauthenticated user can't access the tags API"""
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_filter_tags_not_assigned_to_recipes(self):
        """Test filtering tags by those not assigned to recipes"""
        tag1 = Tag.objects.create(user=self.user, name='Breakfast')
        tag2 = Tag.objects.create(user=self.user, name='Lunch')
        recipe = Recipe.objects.create(
            title='Coriander eggs on toast',
        ...
        res = self.client.get(TAGS_URL, {'assigned_only': 2})
